...
static mg::Pipeline createSolidRendering(const mg::RenderContext &renderContext) {
  ...
  pipelineStateDesc.rasterization.vkPipelineLayout =
  ...
  createPipelineInfo.vertexInputStateCount =
      mg::countof(InputAssembler::vertexInputState);

  const auto pipeline = mg::mgSystem.pipelineContainer.createPipeline(pipelineStateDesc,
                                                                      createPipelineInfo);
  return pipeline;
}
...
void renderSolidBoxes(const mg::RenderContext &renderContext, const float *xPositions,
  ...
  Ubo *ubo = (Ubo *)mg::mgSystem.linearHeapAllocator.allocateUniform(
      sizeof(Ubo), &uniformBuffer, &uniformOffset, &uboSet);
  ubo->mvp = glm::ortho(0.0f, float(vkContext.screen.width), 0.0f,
                        float(vkContext.screen.height), -10.0f, 10.0f);
  VkBuffer storageBuffer;
  uint32_t storageOffset;
  VkDescriptorSet storageSet;
  Storage::StorageData *storage =
      (Storage::StorageData *)mg::mgSystem.linearHeapAllocator.allocateStorage(
          sizeof(Storage::StorageData) * count, &storageBuffer, &storageOffset,
          &storageSet);
  ...
  VertexInputData *vertices =
      (VertexInputData *)mg::mgSystem.linearHeapAllocator.allocateBuffer(
  ...
  vkCmdBindDescriptorSets(mg::vkContext.commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                          solidPipeline.layout, 0, mg::countof(descriptorSets.values),
                          descriptorSets.values, mg::countof(offsets), offsets);

  vkCmdBindPipeline(mg::vkContext.commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                    solidPipeline.pipeline);

  vkCmdBindVertexBuffers(mg::vkContext.commandBuffer, 0, 1, &vertexBuffer,
                         &vertexBufferOffset);
  vkCmdBindIndexBuffer(mg::vkContext.commandBuffer, indexBuffer, indexBufferOffset,
                       VK_INDEX_TYPE_UINT32);
  vkCmdDrawIndexed(mg::vkContext.commandBuffer, mg::countof(indicesInputData), count, 0,
                   0, 0);
} // namespace mg

static mg::Pipeline
createTextureRenderingPipeline(const mg::RenderContext &renderContext) {
...
void renderBoxWithTexture(mg::RenderContext &renderContext, const glm::vec4 &position,
  ...
  uint32_t uniformOffset;
  VkDescriptorSet uboSet;
  Ubo *dynamic = (Ubo *)mg::mgSystem.linearHeapAllocator.allocateUniform(
      sizeof(Ubo), &uniformBuffer, &uniformOffset, &uboSet);
  ...
  vkCmdPushConstants(mg::vkContext.commandBuffer, texturePipeline.layout,
                     VK_SHADER_STAGE_ALL, 0, sizeof(TextureIndices), &textureIndices);
  ...
  vkCmdBindDescriptorSets(mg::vkContext.commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                          texturePipeline.layout, 0, mg::countof(descriptorSets.values),
                          descriptorSets.values, mg::countof(dynamicOffsets),
                          dynamicOffsets);
  vkCmdBindPipeline(mg::vkContext.commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
  ...
  vkCmdBindIndexBuffer(mg::vkContext.commandBuffer, indexBuffer, indexBufferOffset,
                       VK_INDEX_TYPE_UINT32);
  vkCmdDrawIndexed(mg::vkContext.commandBuffer, mg::countof(indicesInputData), 1, 0, 0,
                   0);
}

static mg::Pipeline
createDepthTextureRenderingPipeline(const mg::RenderContext &renderContext) {
...
void renderCube(const mg::RenderContext &renderContext, mg::MeshId cubeId,
  ...
  uint32_t dynamicOffsets[] = {uniformOffset, 0};
  vkCmdBindDescriptorSets(mg::vkContext.commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                          pipeline.layout, 0, mg::countof(descriptorSets.values),
                          descriptorSets.values, mg::countof(dynamicOffsets),
                          dynamicOffsets);
...
void renderFluid(const mg::RenderContext &renderContext, float *density, size_t size) {
