...
DeviceHeapAllocation DeviceMemoryAllocator::allocateLargeDeviceOnlyMemory(uint32_t memoryTypeIndex, VkDeviceSize sizeInBytes, VkDeviceSize alignment) {
  const auto alignSize = mg::alignUpPowerOfTwo(sizeInBytes, alignment);
  int32_t index = -1;
  ...
  vkMemoryAllocateInfo.allocationSize = alignSize;
  vkMemoryAllocateInfo.memoryTypeIndex = memoryTypeIndex;
  checkResult(vkAllocateMemory(mg::vkContext.device, &vkMemoryAllocateInfo, nullptr, &_largeSizeAllocations[index].deviceMemory));

  DeviceHeapAllocation deviceHeapAllocation = {};
  deviceHeapAllocation.alignSize = alignSize;
  ...
  _largeSizeAllocations[index].size = alignSize;
...
DeviceHeapAllocation DeviceMemoryAllocator::allocateDeviceOnlyMemory(uint32_t memoryTypeIndex, VkDeviceSize sizeInBytes, VkDeviceSize alignment) {
  ...
  for(; heapIndex < NR_OF_HEAPS; heapIndex++) {
    ...
    if(baseNode->next == nullptr && _allocationInfos[memoryTypeIndex][heapIndex].totalSize == 0) {
      ...
      checkResult(vkAllocateMemory(mg::vkContext.device, &vkMemoryAllocateInfo, nullptr, &_deviceMemories[memoryTypeIndex][heapIndex]));
    ...
    for(mg::_Node *prevNode = baseNode, *currentNode = baseNode->next; currentNode != nullptr; prevNode = currentNode, currentNode = currentNode->next) {
      const auto alignSize = mg::alignUpPowerOfTwo(sizeInBytes, alignment);
      
      if(alignSize <= currentNode->size) {
        ...
        allocation.alignSize = alignSize;
        allocation.offset = currentNode->offset;

        if(alignSize == currentNode->size) {
          prevNode->next = currentNode->next;
          delete currentNode;
        }
        else { // alignSize < currentNode->size
          currentNode->size = currentNode->size - alignSize;
          currentNode->offset += alignSize;
        }
        _allocationInfos[memoryTypeIndex][heapIndex].currentSize += alignSize;
...
void DeviceMemoryAllocator::freeDeviceOnlyMemory(const DeviceHeapAllocation &allocation) {
  ...
  _allocationInfos[memoryTypeIndex][heapIndex].currentSize -= allocation.alignSize;
  _allocationInfos[memoryTypeIndex][heapIndex].allocationNotFreed--;
  mgAssert(_allocationInfos[memoryTypeIndex][heapIndex].currentSize >= 0);
  mgAssert(_allocationInfos[memoryTypeIndex][heapIndex].allocationNotFreed >= 0);
  ...
  for(; currentNode != nullptr; prevNode = currentNode, currentNode = currentNode->next) {
    ...
    if(allocation.offset < currentNode->offset) {
      if(allocation.offset + allocation.alignSize == currentNode->offset) {
        currentNode->offset = allocation.offset;
        currentNode->size += allocation.alignSize;
      }
      else {
        currentNode = nullptr;
      }
...
std::vector<GuiAllocation> DeviceMemoryAllocator::getAllocationForGUI() {
  ...
  for(uint32_t memoryTypeIndex = 0; memoryTypeIndex < _nrOfHeapTypes; memoryTypeIndex++) {
    for(uint32_t heapIndex = 0; heapIndex < NR_OF_HEAPS; heapIndex++) {
      ...
      for(_Node *currentNode = baseNode->next; currentNode != nullptr; prevNode = currentNode, currentNode = currentNode->next) {
