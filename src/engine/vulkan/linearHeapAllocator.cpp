...
static constexpr struct {
  ...
  VkBufferUsageFlags vertexBufferUsageFlags = VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT;
} usageFlags;

namespace mg {

static mg::_Buffer _createLinearBuffer(uint32_t bufferSizeInBytes, VkBufferUsageFlags vkBufferUsageFlags, VkMemoryPropertyFlags requiredProperties, VkMemoryPropertyFlags preferredProperties) {
  mg::_Buffer buffer = {};
  ...
  const auto memoryTypeIndex = findMemoryTypeIndex(mg::vkContext.physicalDeviceMemoryProperties, vkMemoryRequirements.memoryTypeBits, requiredProperties, preferredProperties);
  ...
  for(uint32_t i = 0; i < NrOfBuffers; i++)
    checkResult(vkBindBufferMemory(mg::vkContext.device, buffer.bufferViews[i].vkBuffer, buffer.deviceMemory, alignedSize * i));
...
static void* allocateDynamicBuffer(mg::_Buffer *dynamicBuffer, uint32_t currentBufferIndex, VkDeviceSize sizeInBytes, VkBuffer *buffer, VkDeviceSize *offset) {
  ...
  *offset = dynamicBuffer->bufferViews[currentBufferIndex].offset;
  ...
  char *data = dynamicBuffer->bufferViews[currentBufferIndex].data + dynamicBuffer->bufferViews[currentBufferIndex].offset;
...
LinearHeapAllocator::~LinearHeapAllocator() {
  mgAssert(_hasBeenDelete == true);
}

void* LinearHeapAllocator::allocateBuffer(VkDeviceSize sizeInBytes, VkBuffer *buffer, VkDeviceSize *offset) {
  return allocateDynamicBuffer(&_vertexBuffer, _currentBufferIndex, sizeInBytes, buffer, offset);
}

void* LinearHeapAllocator::allocateUniform(VkDeviceSize sizeInBytes, VkBuffer *buffer, uint32_t *offset, VkDescriptorSet *vkDescriptorSet) {
...
void* LinearHeapAllocator::allocateLargeStaging(VkDeviceSize sizeInBytes, VkCommandBuffer *commandBuffer, VkBuffer *buffer, VkDeviceSize *offset) {
  ...
  // VK_MEMORY_PROPERTY_HOST_COHERENT_BIT is not cached and does not need be flushed
  const auto memoryTypeIndex = findMemoryTypeIndex(mg::vkContext.physicalDeviceMemoryProperties, vkMemoryRequirements.memoryTypeBits, usageFlags.requiredProperties);
  const auto alignedSize = vkMemoryRequirements.size;
  ...
  *commandBuffer = allocation->commandBuffer;
  allocation->alignSize = alignedSize;

  return data;
}

void* LinearHeapAllocator::allocateStaging(VkDeviceSize sizeInBytes, VkDeviceSize alignmentOffset, VkCommandBuffer *commandBuffer, VkBuffer *buffer, VkDeviceSize *offset) {
  ...
  const auto alignOffset = mg::alignUpPowerOfTwo(dynamicBuffer.bufferViews[_currentBufferIndex].offset, alignmentOffset);
  ...
  if(stagingBuffer.submitted[_currentBufferIndex]) {
    ...
    checkResult(vkWaitForFences(mg::vkContext.device, 1, &stagingBuffer.vkFences[_currentBufferIndex], VK_TRUE, UINT64_MAX));
...
void LinearHeapAllocator::create() {
  ...
  _stagingBuffer.buffer = _createLinearBuffer(stagingBufferSizeInBytes, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, usageFlags.requiredProperties, 0);

  checkResult(vkAllocateCommandBuffers(mg::vkContext.device, &vkCommandBufferAllocateInfo, _stagingBuffer.vkCommandBuffers));
...
void LinearHeapAllocator::submitStagingMemoryToDeviceLocalMemory() {
  if(_stagingBuffer.buffer.bufferViews[_currentBufferIndex].offset > 0 && _stagingBuffer.submitted[_currentBufferIndex] == false) {
...
void LinearHeapAllocator::swapLinearHeapBuffers() {
  {
    ...
    _maxStagingSize = std::max(_maxStagingSize, uint32_t(_stagingBuffer.buffer.bufferViews[_currentBufferIndex].offset));
    for(const auto &allocation : _largeLinearStagingAllocation) {
      _largeAllocationHistory.push_back(allocation.alignSize);
    }
  }
  submitStagingMemoryToDeviceLocalMemory();
...
std::vector<GuiAllocation> LinearHeapAllocator::getAllocationForGUI() {
...

  {
    ...
    if(subAllocationGui.size) vertex.elements.push_back(subAllocationGui);
  ...
  }
  {
    ...
    if(subAllocationGui.size) staging.elements.push_back(subAllocationGui);

    SubAllocationGui subAllocationGuiEmpty = {};
    subAllocationGuiEmpty.offset = subAllocationGui.size;
    subAllocationGuiEmpty.free = true;
    subAllocationGuiEmpty.size = stagingBufferSizeInBytes - _maxStagingSize;
    if(subAllocationGuiEmpty.size) staging.elements.push_back(subAllocationGuiEmpty);
  }
