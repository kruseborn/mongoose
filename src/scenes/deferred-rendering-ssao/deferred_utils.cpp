...
Noise createNoise() {
  ...
  const auto kernalSize = mg::countof(noise.ssaoKernel.kernel);
  for (uint32_t i = 0; i < kernalSize; ++i) {
    ...
    sample *= scale;
    noise.ssaoKernel.kernel[i] = {sample, 1.0f};
  }
  ...
  noise.noiseTexture = mg::mgSystem.textureContainer.createTexture(createTextureInfo);

  noise.ssaoKernel.noiseScale = glm::vec2(mg::vkContext.screen.width / 4, mg::vkContext.screen.height / 4);
