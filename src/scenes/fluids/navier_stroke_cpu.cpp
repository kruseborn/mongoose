...
void diffuseCompute2(int N, int b, float *x, float *x0, float diff, float dt) {
...
void advectCompute2(int N, int b, float *d, float *d0, float *u, float *v, float dt) {
  auto compute = createCompute();
...
void preProjectCompute2(int N, float *u, float *v, float *p, float *div) {
...
void projectCompute2(int N, float *u, float *v, float *p, float *div) {
...
void postProjectCompute2(int N, float *u, float *v, float *p, float *div) {
...
void project_C2(int N, float *u, float *v, float *p, float *div) {
  // for (i = 1; i <= N; i++) {
  //  for (j = 1; j <= N; j++) {
  ...
  // set_bnd(N, 0, div);
  // set_bnd(N, 0, p);
  preProjectCompute2(N, u, v, p, div);
  // mg::waitForDeviceIdle();

  projectCompute2(N, u, v, p, div);
  // for (k = 0; k < 20; k++) {
  //  for (i = 1; i <= N; i++) {
  //    for (j = 1; j <= N; j++) {
  ...
  //  set_bnd(N, 0, p);
  //}
  postProjectCompute2(N, u, v, p, div);
}
...
void vel_step(int N, float *u, float *v, float *u0, float *v0, float visc, float dt) {
  {
    ...
    diffuseCompute2(N, 1, u0, u, visc, dt);
    diffuseCompute2(N, 2, v0, v, visc, dt);
...

  {
    ...
    advectCompute2(N, 1, u, u0, u0, v0, dt);
    advectCompute2(N, 2, v, v0, u0, v0, dt);
    auto end = mg::timer::now();
    fluidTimes.advecSum += uint32_t(mg::timer::durationInMs(start, end));
  }

  project_C2(N, u, v, u0, v0);

  // density step
  diffuseCompute2(N, 0, s, dens, diff, dt);
  advectCompute2(N, 0, dens, s, u, v, dt);
}
...
void renderNavierStroke2(const mg::RenderContext &renderContext) { renderFluid2(renderContext, dens, size); }
